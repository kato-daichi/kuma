...
#else
	return CreateFile(name, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_FLAG_RANDOM_ACCESS, nullptr);
#endif
}
//-------------------------------------------//
...
namespace anonymous
{
	...
	//-------------------------------------------//
	void half_kp_append_changed_indices(const board* pos, const int c, const dirty_piece* dp, index_list* removed,
		index_list* added)
	{
		int ksq = pos->squares[c];
		ksq = orient(c, ksq);
	...
	//-------------------------------------------//
	void append_changed_indices(const board* pos, index_list removed[2], index_list added[2], bool reset[2])
	{
...
//-------------------------------------------//
inline bool next_idx(unsigned* idx, unsigned* offset, mask2_t* v, mask_t* mask, const unsigned in_dims)
{
	while (*v == 0)
	{
...
//-------------------------------------------//
inline void affine_txfm(int8_t* input, void* output, unsigned in_dims, unsigned out_dims, const int32_t* biases,
                        const weight_t* weights, mask_t* in_mask, mask_t* out_mask, const bool pack8_and_calc_mask)
{
	assert(out_dims == 32);
	(void)out_dims;
...
inline void refresh_accumulator(const board* pos)
{
	...
	for (unsigned c = 0; c < 2; c++)
	{
		...
		{
			const vec16_t* ft_biases_tile = reinterpret_cast<vec16_t*>(&anonymous::ft_biases[i * TILE_HEIGHT]);
			...
			for (size_t k = 0; k < active_indices[c].size; k++)
			{
				...
				const vec16_t* column = reinterpret_cast<vec16_t*>(&anonymous::ft_weights[offset]);
				for (unsigned j = 0; j < num_regs; j++)
...
inline bool update_accumulator(const board* pos)
{
	...
	for (unsigned i = 0; i < k_half_dimensions / TILE_HEIGHT; i++)
	{
		for (unsigned c = 0; c < 2; c++)
		{
			...
			if (reset[c])
			{
				const vec16_t* ft_b_tile = reinterpret_cast<vec16_t*>(&anonymous::ft_biases[i * TILE_HEIGHT]);
				for (unsigned j = 0; j < num_regs; j++)
					acc[j] = ft_b_tile[j];
			}
			else
			{
				const vec16_t* prev_acc_tile = reinterpret_cast<vec16_t*>(&prev_acc->accumulation[c][i * TILE_HEIGHT]);
				for (unsigned j = 0; j < num_regs; j++)
...
#define B(x) (buf.x)
	...
	const int32_t out_value = affine_propagate(B(hidden2_out), output_biases, output_weights);
