...
int alpha_beta(searchthread* thread, searchinfo* info, int depth, int alpha, int beta)
{
		...
		(!tt_hit || !(flag & flag_alpha) || hash_score >= beta))
	{
		...
		info->counter_move_history = &thread->counter_move_history[blank][0];
...
{
	is_movetime = limits.timelimited;
	is_depth = limits.depthlimited;
	is_infinite = limits.infinite;
	is_timeout = false;
	depth_limit = limits.depthlimited ? limits.depthlimit : max_ply;
...
bool is_draw(Position* pos)
{
	...
	if (halfmove_clock > 3)
	{
		...
		const int rootheight = pos->my_thread->rootheight;
...
//-------------------------------------------//
void print_info(const Position* pos, const searchinfo* info, const int depth, int score, const int alpha, const int beta)
{
...
int q_search_delta(const Position* pos)
{
	...
	return delta + (pos->piece_bb[wqueen | enemy]
		                ? queen_mg
		                : pos->piece_bb[wrook | enemy]
		                ? rook_mg
		                : pos->piece_bb[wbishop | enemy]
		                ? bishop_mg
		                : pos->piece_bb[wknight | enemy]
		                ? knight_mg
		                : pawn_mg);
}
//-------------------------------------------//
