...
void go()
{
	...
	if (args.size() <= 1)
	{
		...
		limits.movetime = 0;
		limits.depthlimit = 0;
		limits.timelimited = true;
		limits.depthlimited = false;
	...
	else
	{
		...
		for (unsigned int i = 1; i < args.size(); ++i)
		{
			if (args[i] == "wtime")
			{
				wtime = stoi(args[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(i) + 1]);
			}
			else if (args[i] == "btime")
			{
				btime = stoi(args[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(i) + 1]);
			}
			else if (args[i] == "winc")
			{
				winc = stoi(args[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(i) + 1]);
			}
			else if (args[i] == "binc")
			{
				binc = stoi(args[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(i) + 1]);
			}
			else if (args[i] == "movestogo")
			{
				movestogo = stoi(args[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(i) + 1]);
			}
			else if (args[i] == "depth")
			{
				depthlimited = true;
				depth = stoi(args[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(i) + 1]);
			}
			...
			{
				movetime = stoi(args[static_cast<std::vector<std::string, std::allocator<std::string>>::size_type>(i) + 1]) * 99 / 100;
				timelimited = true;
			}
		}
		...
		limits.timelimited = timelimited;
		limits.depthlimited = depthlimited;
...
move uci2_move(const Position* pos, const std::string& s)
{
	...
	else if (pc >= wking && (abs(from - to) == 2 || BITSET(to) & pos->piece_bb[make_piece(pos->active_side, rook)]))
	{
		if (abs(from - to) == 2)
