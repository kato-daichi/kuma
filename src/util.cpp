...
{
	const char file = static_cast<char>(s[0] - 'a');
	const char rank = static_cast<char>(s[1] - '1');

	return static_cast<unsigned char>(rank << 3 | file);
}
//-------------------------------------------//
...
void bench()
{
	...
	limits.increment = 0;
	limits.movetime = 0;
	limits.depthlimit = 16;
	limits.timelimited = false;
	limits.depthlimited = true;
