...
s_move* generate_piece_moves(const Position& pos, s_move* movelist, const uint64_t occ, const uint64_t to_squares, const uint64_t from_mask)
{
	...
	while (from_squares)
	{
		...
		if (Pt == king)
			tobits = pseudo_attacks[king][from] & to_squares & ~pos.taboo_squares();
		while (tobits)
		{
...
{
	const int opponent = active_side ^ sideswitch;
	const uint64_t rooks_queens = piece_bb[wrook | opponent] | piece_bb[wqueen | opponent];
...
s_move* generate_pawn_moves(const Position& pos, s_move* movelist, const uint64_t from_mask, uint64_t to_mask)
{
	...
	constexpr auto pc = static_cast<piece_code>(wpawn | Side);
	...
	if (Mt == quiet_check)
	{
		uint64_t target = pawn_attacksfrom[Side][pos.kingpos[opponent]];
	...
	if (Mt & capture)
	{
		...
		if (pos.ep_square)
		{
			...
			while (b1)
			{
				if (const int from = pop_lsb(b1); RANK(from) != RANK(pos.kingpos[Side]) || !pos.horizontal_check(occ ^ BITSET(from) ^ BITSET(pos.ep_square + (Side ? 8 : -8)), pos.kingpos[Side]))
...
s_move* generate_castles(const Position& pos, s_move* movelist)
{
	...
	{
		if ((pos.castle_rights & wqcmask << index) == 0)
			continue;
		...
		if (between_masks[king_from][rook_from] & occupied)
			continue;
		uint64_t attack_targets = castlekingwalk[index];
		bool attacked = false;
		while (!attacked && attack_targets)
		{
			const int to = pop_lsb(attack_targets);
			attacked = pos.is_attacked(to, Side ^ sideswitch);
		}
		if (attacked)
			continue;
...
s_move* generate_pinned_moves(const Position& pos, s_move* movelist)
{
	...
	while (diag_pinned)
	{
		...
		else if (pt == pawn)
		{
			...
			{
				if (BITSET(pos.ep_square) & target_mask)
				{
					uint64_t from_bb = pos.piece_bb[wpawn | Side] & pawn_attacks[opponent][pos.ep_square] & pinned;
					while (from_bb)
					{
...
void Position::update_blockers()
{
	...
	{
		const int opponent = us ^ sideswitch;
...
s_move* generate_pseudo_legal_moves(const Position& pos, s_move* movelist, const uint64_t from_mask)
{
	...
	if (Mt & quiet)
		to_squares |= ~occ;
	if (Mt & capture)
		to_squares |= pos.occupied_bb[pos.active_side ^ sideswitch];
...
s_move* generate_evasions(const Position& pos, s_move* movelist)
{
	...
	if (ONEORZERO(pos.check_bb))
	{
		...
		while (from_bb)
		{
			if (const int from = pop_lsb(from_bb); pos.mailbox[from] <= bpawn && PROMOTERANK(checker))
			{
		...
		if (pos.ep_square && pos.ep_square == checker + S2_M_SIGN(Side) * 8)
		{
			from_bb = pawn_attacksfrom[Side][pos.ep_square] & pos.piece_bb[wpawn | Side] & no_pin;
			while (from_bb)
			{
				const int from = pop_lsb(from_bb);
				*movelist++ = make_move(from, checker + S2_M_SIGN(Side) * 8, enpassant);
			}
		}
		...
		while (targets)
		{
			...
			while (from_bb)
			{
				if (const int from = pop_lsb(from_bb); pos.mailbox[from] <= bpawn && PROMOTERANK(to))
				{
					*movelist++ = make_promotion_move(from, to, queen, promotion);
					*movelist++ = make_promotion_move(from, to, knight, promotion);
					*movelist++ = make_promotion_move(from, to, bishop, promotion);
...
s_move* generate_quiet_checks(const Position& pos, s_move* movelist)
{
	...
	while (diag_pinned)
	{
		...
		if (const auto pt = static_cast<piece_type>(pos.mailbox[from] >> 1); pt == bishop)
			movelist = generate_piece_moves<bishop>(pos, movelist, occ, target_mask & bishop_squares, BITSET(from));
		else if (pt == queen)
			movelist = generate_piece_moves<queen>(pos, movelist, occ, target_mask & queen_squares, BITSET(from));
	}
	...
	while (straight_pinned)
	{
		...
		target_mask = ray_masks[my_king][from] & empty_squares;
		if (const auto pt = static_cast<piece_type>(pos.mailbox[from] >> 1); pt == rook)
			movelist = generate_piece_moves<rook>(pos, movelist, occ, target_mask & rook_squares, BITSET(from));
		else if (pt == queen)
			movelist = generate_piece_moves<queen>(pos, movelist, occ, target_mask & queen_squares, BITSET(from));
		else if (pt == pawn && FILE(my_king) == FILE(from))
		{
			movelist = generate_pawn_moves<Side, quiet_check>(pos, movelist, BITSET(from));
		}
	}
	...
	while (nonpinned_fossils)
	{
		...
		{
		case knight:
			movelist = generate_piece_moves<knight>(pos, movelist, occ, target_mask, BITSET(from));
			break;
		case bishop:
			movelist = generate_piece_moves<bishop>(pos, movelist, occ, target_mask, BITSET(from));
			break;
		case rook:
			movelist = generate_piece_moves<rook>(pos, movelist, occ, target_mask, BITSET(from));
			break;
		case queen:
			movelist = generate_piece_moves<queen>(pos, movelist, occ, target_mask, BITSET(from));
			break;
		case king:
			movelist = generate_piece_moves<king>(pos, movelist, occ, target_mask, BITSET(from));
			break;
		case pawn:
			movelist = generate_pawn_moves<Side, quiet>(pos, movelist, BITSET(from), target_mask);
			break;
		default: ;
		}
	}
